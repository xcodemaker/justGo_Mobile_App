...
public class LankagateAPIService {

    private LankagateAPI lankagateAPI;

    public LankagateAPIService() {
        Retrofit restAdapter = new Retrofit.Builder()
                .baseUrl(DomainConstants.LANKAGATE_SERVER_URL)
                .client(getClient())
                .addConverterFactory(GsonConverterFactory.create(new GsonBuilder().disableHtmlEscaping().create()))
                .addCallAdapterFactory(RxErrorHandlingCallAdapterFactory.create())
                .build();

        lankagateAPI = restAdapter.create(LankagateAPI.class);
    }
